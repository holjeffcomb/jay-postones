    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
      {lessons.map((course) => (
        <Link href={`/lessons/${course._id}`} key={course._id}>
          <div className="flex flex-col bg-[var(--secondary-color)] rounded-lg shadow-[4px_4px_4px_rgba(0,0,0,0.8)] overflow-hidden transition-all duration-300 ease-in-out hover:scale-[1.02] hover:shadow-lg hover:bg-[#3D3F5D] group min-w-[300px] w-full h-[460px]">
            {course.imageUrl && (
              <div className="relative w-full pt-[56.25%]">
                ...
                />
                <div
                  className={`absolute top-0 right-0 px-2 rounded-sm m-2 text-[var(--text-color)] ${course.membershipLevel === "free" ? "bg-blue-500" : course.membershipLevel === "silver" ? "bg-gray-500" : course.membershipLevel === "gold" ? "bg-yellow-500" : course.membershipLevel === "platinum" ? "bg-purple-500" : "bg-[var(--accent-color)]"}`}
                >
